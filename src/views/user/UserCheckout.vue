          ...
          <div class="my-5 row d-flex justify-content-center">
            <form class="border p-4 mb-4 border-dark" @submit.prevent="payOrder">
              ...
              <table class="table align-middle mt-4
               border-bottom text-muted border-dark">
                <tbody>
                  <tr v-for="item in order.products" :key="item.id">
                      <img :src="item.product.imageUrl" class="me-2"
                       style="width: 100px; height: 100px; object-fit: cover">
                      <td class="fw-bold">{{ item.product.title }}</td>
                      <td class="text-end">NT${{ $filters.currency(item.final_total) }}</td>
                      <td class="text-secondary text-end" style="white-space: nowrap;">
                        X{{ item.qty }}{{ item.product.unit }}</td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr>
